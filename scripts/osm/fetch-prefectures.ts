 ...
 *
 * 使い方: npx tsx scripts/osm/fetch-prefectures.ts
 */
...
async function main() {
  ...
  try {
    ...
    for (const pref of prefectures) {
      console.log(`  - ${pref.id}: ${pref.name} (${pref.name_translations?.en ?? 'N/A'}) @ ${pref.latitude.toFixed(4)}, ${pref.longitude.toFixed(4)}`);
    }
