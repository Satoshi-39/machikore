-- search_public_spots と search_public_maps に date_range フィルターを追加
--
-- 変更内容:
-- - date_range パラメータ追加: 'all' | 'day' | 'week' | 'month'
-- - search_public_maps に sort_by パラメータ追加

-- ============================================
-- search_public_spots 更新
-- ============================================
-- 既存の関数を全て削除（シグネチャ違いを含む）
DROP FUNCTION IF EXISTS public.search_public_spots(TEXT, INTEGER);
DROP FUNCTION IF EXISTS public.search_public_spots(TEXT, INTEGER, TEXT, TEXT, UUID[], TEXT);
  ...
  SELECT
    ...
    COALESCE(
      (
        ...
        FROM spot_images si
        WHERE si.user_spot_id = fs.id AND si.cloud_path IS NOT NULL
      ),
      ARRAY[]::TEXT[]
    ) AS image_urls
...
-- ============================================
-- search_public_maps 更新（sort_by + date_range 追加）
-- ============================================
