...
-- 混合フィードにショート動画URLを追加
-- ============================================================
-- spot_shortsテーブルから最初の動画URLを取得してフィードに含める
-- 最終更新: 2026-01-20
...
-- 既存の型を削除して再作成（ALTER TYPEでカラム追加は複雑なため）
...
AS $$
  ...
  FROM generate_series(0, p_block_count - 1) AS n
...
BEGIN
  ...
  -- フォロー中ユーザーの公開スポットを取得（spot_video_url追加）
  spots_cte AS (
    SELECT
      'spot'::TEXT AS item_type,
      s.id AS item_id,
