-- ============================================================
-- mixed_feed_item複合型からis_article_publicを削除
-- ============================================================
-- maps.is_article_publicカラムが削除されたため、
-- RPC関数と複合型からも参照を削除する
--
-- 手順:
-- 1. RPC関数を削除（複合型に依存しているため）
-- 2. 広告スロット生成関数を削除
-- 3. mixed_feed_item複合型を削除
-- 4. 複合型を再作成（map_is_article_publicなし）
-- 5. 広告スロット生成関数を再作成
-- 6. RPC関数を再作成
...
-- 1. RPC関数を削除
-- ============================================================
DROP FUNCTION IF EXISTS public.get_mixed_feed(INT, INT, TIMESTAMPTZ, TIMESTAMPTZ, UUID, BOOLEAN, INT, BOOLEAN);
DROP FUNCTION IF EXISTS public.get_following_mixed_feed(UUID, INT, INT, TIMESTAMPTZ, TIMESTAMPTZ, BOOLEAN, INT, BOOLEAN);
...
-- 3. mixed_feed_item複合型を削除
-- ============================================================
DROP TYPE IF EXISTS public.mixed_feed_item;

-- ============================================================
-- 4. mixed_feed_item複合型を再作成（map_is_article_publicなし）
-- ============================================================
CREATE TYPE public.mixed_feed_item AS (
  ...
  -- マップフィールド（map_is_article_public削除）
  map_id UUID,
  map_name TEXT,
...
-- ============================================================
-- 7. get_following_mixed_feed関数を再作成（map_is_article_public削除）
-- ============================================================
