-- スポット検索RPCにarticle_contentを追加
--
-- 変更内容:
-- search_public_spots: article_contentを戻り値に追加（戻り値の型が変わるためDROP→CREATE）
...
COMMENT ON FUNCTION public.search_public_spots IS '公開スポットをキーワードで検索。description、master_spots.name、user_spots.name（現在地/ピン刺し登録）を検索対象とし、タグ情報とarticle_contentも含めて返す';
