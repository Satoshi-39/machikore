-- ============================================================
-- maps, user_spots, comments に moderation_status カラムを追加
-- 管理者によるコンテンツモデレーション機能の基盤
-- ============================================================

-- ============================================================
-- 1. moderation_status enum型の作成
-- ============================================================
CREATE TYPE public.moderation_status AS ENUM ('normal', 'hidden', 'removed');

COMMENT ON TYPE public.moderation_status IS 'コンテンツのモデレーションステータス: normal=通常, hidden=管理者が非表示, removed=管理者が削除';

-- ============================================================
-- 2. カラム追加
-- ============================================================
ALTER TABLE public.maps
  ADD COLUMN moderation_status public.moderation_status NOT NULL DEFAULT 'normal';
...
-- ============================================================
-- 3. 部分インデックス（moderation_status != 'normal' のみ）
-- ============================================================
CREATE INDEX idx_maps_moderation_status
  ON public.maps (moderation_status)
  WHERE moderation_status != 'normal';

CREATE INDEX idx_user_spots_moderation_status
  ON public.user_spots (moderation_status)
  WHERE moderation_status != 'normal';

CREATE INDEX idx_comments_moderation_status
  ON public.comments (moderation_status)
  WHERE moderation_status != 'normal';

-- ============================================================
-- 4. RLSポリシー更新
-- モデレート済みコンテンツは公開アクセスから除外
-- オーナーは自分のコンテンツを引き続き閲覧可能
-- ============================================================

-- maps: 公開 AND moderation_status=normal、または自分のマップ
DROP POLICY IF EXISTS maps_select_public_or_own ON public.maps;
CREATE POLICY maps_select_public_or_own ON public.maps FOR SELECT
  USING (
    (is_public = true AND moderation_status = 'normal')
    OR ((select auth.uid()) IS NOT NULL AND user_id = (select auth.uid()))
  );

-- user_spots: 公開マップ & 公開スポット & moderation_status=normal、または自分のマップのスポット
DROP POLICY IF EXISTS user_spots_select_public_or_own ON public.user_spots;
CREATE POLICY user_spots_select_public_or_own ON public.user_spots FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM public.maps
      WHERE maps.id = user_spots.map_id AND (
        (maps.is_public = true AND user_spots.is_public = true AND user_spots.moderation_status = 'normal')
        OR (maps.user_id = (select auth.uid()))
      )
    )
  );

-- comments: moderation_status=normal、または自分のコメント
...
SELECT "id",
   ...
   FROM "public"."maps" "m"
  WHERE ("is_public" = true) AND ("moderation_status" = 'normal');

ALTER VIEW "public"."maps_public" OWNER TO "postgres";
ALTER VIEW "public"."maps_public" SET (security_invoker = on);

COMMENT ON VIEW "public"."maps_public" IS '公開マップ一覧（モデレート済み除外、spots_countは公開スポット数のみ）';
...
GRANT SELECT ON "public"."maps_public" TO "service_role";
...
-- ============================================================
-- 7. RPC関数更新
-- SECURITY DEFINER関数はRLSをバイパスするため、
-- moderation_status フィルターを直接追加する必要がある
-- ============================================================

-- 7a. get_following_mixed_feed
-- 変更点:
--   maps_cte: AND m.moderation_status = 'normal' 追加
--   spots_cte: AND s.moderation_status = 'normal' 追加
...
BEGIN
  ...
  spots_cte AS (
    ...
    WHERE s.is_public = true
      AND s.moderation_status = 'normal'
  ...
  maps_with_position AS (
    SELECT
      ...
      NULL::TEXT AS spot_name, NULL::INT AS spot_images_count, NULL::INT AS spot_likes_count,
...
-- 7b. get_mixed_feed
-- 変更点:
--   maps_cte: maps_public ビューが既にフィルター済み（変更不要）
--   spots_cte: AND s.moderation_status = 'normal' 追加
...
-- 7c. search_public_maps
-- 変更点: AND m.moderation_status = 'normal' 追加
DROP FUNCTION IF EXISTS "public"."search_public_maps"("text", integer, "uuid"[], "text", "text", "text", "uuid", integer);
...
COMMENT ON FUNCTION "public"."search_public_maps" IS '公開マップ検索。モデレート済み除外。フィルター: タグ、期間、地域テキスト、ブロックユーザー除外。並び替え: created_at / likes_count。ページネーション対応。';

GRANT ALL ON FUNCTION "public"."search_public_maps"("text", integer, "uuid"[], "text", "text", "text", "uuid", integer) TO "anon";
GRANT ALL ON FUNCTION "public"."search_public_maps"("text", integer, "uuid"[], "text", "text", "text", "uuid", integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_public_maps"("text", integer, "uuid"[], "text", "text", "text", "uuid", integer) TO "service_role";

-- 7d. search_public_spots
-- 変更点: AND us.moderation_status = 'normal' 追加
DROP FUNCTION IF EXISTS "public"."search_public_spots"("text", integer, "text", "text", "uuid"[], "text", "text", "uuid", integer);
...
BEGIN
  ...
  WITH filtered_spots AS (
    ...
    WHERE
      us.is_public = true
      AND us.moderation_status = 'normal'
      AND (
        search_query IS NULL
        OR search_query = ''
        OR (
          ...
          OR (us.name IS NOT NULL AND us.name ILIKE '%' || search_query || '%')
        )
      )
...
COMMENT ON FUNCTION "public"."search_public_spots" IS '公開スポット検索。モデレート済み除外。フィルター: 都道府県、市区町村、タグ、期間、ブロックユーザー除外。並び替え: created_at / likes_count。ページネーション対応。';

GRANT ALL ON FUNCTION "public"."search_public_spots"("text", integer, "text", "text", "uuid"[], "text", "text", "uuid", integer) TO "anon";
GRANT ALL ON FUNCTION "public"."search_public_spots"("text", integer, "text", "text", "uuid"[], "text", "text", "uuid", integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_public_spots"("text", integer, "text", "text", "uuid"[], "text", "text", "uuid", integer) TO "service_role";
