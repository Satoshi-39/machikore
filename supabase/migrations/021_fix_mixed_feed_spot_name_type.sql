-- user_spots.name の JSONB→TEXT 変更に伴う関連オブジェクトの修正
-- (1) mixed_feed_item 複合型の spot_name を text に変更
-- (2) search_public_spots の修正（020で誤ったオーバーロードが作成された問題の修正）

-- =============================================
-- A. search_public_spots の修正
-- =============================================
-- migration 020 でパラメータ型が変わり（uuid[]→text[], date_range→date_range_filter）
-- PostgreSQLが新しいオーバーロードを作成してしまった。
-- 不要なオーバーロードを削除し、正しいシグネチャで再作成する。

-- 020で作成された不正なオーバーロード（text[]シグネチャ）を削除
DROP FUNCTION IF EXISTS public.search_public_spots(text, text, text, text[], text, text, int, text, uuid);

-- 元の正しいシグネチャの関数も削除（RETURNS TABLEを更新するため）
DROP FUNCTION IF EXISTS public.search_public_spots(text, integer, text, text, uuid[], text, text, uuid);

-- 正しいシグネチャで再作成（name→text, language追加、検索条件修正）
...
-- 2. mixed_feed_item の spot_name を text に変更
ALTER TYPE "public"."mixed_feed_item" ALTER ATTRIBUTE "spot_name" TYPE text;

-- 3. generate_feed_ad_slots を再作成（spot_name を NULL::TEXT に変更）
...
-- 4. get_following_mixed_feed を再作成（spot_name を NULL::TEXT に変更）
...
-- 5. get_mixed_feed を再作成（spot_name を NULL::TEXT に変更）
