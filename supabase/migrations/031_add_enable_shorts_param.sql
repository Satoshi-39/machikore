-- ============================================================
-- ショート表示の有効/無効切り替えパラメータを追加
-- ============================================================
-- p_enable_shorts パラメータを追加し、デフォルトはFALSE（cardのみ）
-- フェーズ2でショートを有効化する際はTRUEに変更
-- 最終更新: 2026-01-21
...
CREATE OR REPLACE FUNCTION public.get_mixed_feed(
  ...
  p_enable_shorts BOOLEAN DEFAULT FALSE  -- ショート表示の有効/無効（フェーズ2で有効化）
)
...
BEGIN
  ...
  spots_with_position AS (
    SELECT
      item_type, item_id, created_at,
      -- ショート有効時のみshort/card交互、無効時は常にcard
  ...
  SELECT
    ...
    c.ad_slot,
    c.spot_display_type,
    c.map_id, c.map_name, c.map_description, c.map_thumbnail_url, c.map_is_public, c.map_is_article_public,
