-- user_spots.name を JSONB から TEXT に変更
-- user_spots.language を NOT NULL（デフォルト 'ja'）に変更

-- 1. name カラムを JSONB → TEXT に変更（一時カラム方式）

-- 1a. 一時的なTEXTカラムを追加
ALTER TABLE user_spots ADD COLUMN name_text text;

-- 1b. languageキーで抽出できるものを変換
UPDATE user_spots
SET name_text = name ->> COALESCE(language, 'ja')
WHERE name IS NOT NULL
  AND name ->> COALESCE(language, 'ja') IS NOT NULL;

-- 1c. languageキーで抽出できなかったもの（他の言語キーのみ）は最初の値を使用
UPDATE user_spots us
SET name_text = sub.first_value
FROM (
  SELECT us2.id, (SELECT value FROM jsonb_each_text(us2.name) LIMIT 1) AS first_value
  FROM user_spots us2
  WHERE us2.name IS NOT NULL
    AND us2.name_text IS NULL
) sub
WHERE us.id = sub.id;

-- 1d. 元のnameカラムを削除して、name_textをnameにリネーム
ALTER TABLE user_spots DROP COLUMN name;
ALTER TABLE user_spots RENAME COLUMN name_text TO name;

-- 2. language カラムを NOT NULL に変更（デフォルト 'ja'）
UPDATE user_spots SET language = 'ja' WHERE language IS NULL;
ALTER TABLE user_spots ALTER COLUMN language SET NOT NULL;
ALTER TABLE user_spots ALTER COLUMN language SET DEFAULT 'ja';

-- 3. search_public_spots RPC の更新
-- ※注意: この定義はパラメータシグネチャが元と異なるため、
-- PostgreSQLが新しいオーバーロードを作成してしまう問題がある。
-- 修正は 021_fix_mixed_feed_spot_name_type.sql で行う。
-- (以下は適用済みだが問題あり)
CREATE OR REPLACE FUNCTION search_public_spots(
  search_query text DEFAULT NULL,
  prefecture_id_filter text DEFAULT NULL,
  city_id_filter text DEFAULT NULL,
  tag_ids_filter text[] DEFAULT NULL,
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object('id', t.id, 'name', t.name, 'slug', t.slug))
       FROM spot_tags st JOIN tags t ON t.id = st.tag_id
    ...
    -- images
    COALESCE(
      (SELECT array_agg(i.cloud_path ORDER BY i.order_index)
       FROM images i
       WHERE i.user_spot_id = us.id AND i.cloud_path IS NOT NULL),
      ARRAY[]::text[]
    ) AS image_urls,
    -- thumbnail
  ...
  WHERE us.is_public = true
    ...
    AND (search_query IS NULL OR search_query = '' OR
      us.description ILIKE '%' || search_query || '%' OR
      us.name ILIKE '%' || search_query || '%' OR
      EXISTS (
        SELECT 1 FROM master_spots ms2
        WHERE ms2.id = us.master_spot_id
        AND ms2.name::text ILIKE '%' || search_query || '%'
      )
    )
    -- prefecture filter
    ...
    -- tag filter
    AND (tag_ids_filter IS NULL OR array_length(tag_ids_filter, 1) IS NULL OR
      EXISTS (
        SELECT 1 FROM spot_tags st
        WHERE st.user_spot_id = us.id AND st.tag_id = ANY(tag_ids_filter)
      )
    )
    -- date range filter
    AND (
      date_range_filter = 'all' OR
      (date_range_filter = 'day' AND us.created_at >= NOW() - INTERVAL '1 day') OR
      (date_range_filter = 'week' AND us.created_at >= NOW() - INTERVAL '1 week') OR
      (date_range_filter = 'month' AND us.created_at >= NOW() - INTERVAL '1 month')
    )
    -- cursor pagination
    AND (cursor_value IS NULL OR
      (sort_by = 'created_at' AND us.created_at < cursor_value::timestamptz) OR
      (sort_by = 'likes_count' AND (us.likes_count, us.created_at) < (
        (SELECT us2.likes_count FROM user_spots us2 WHERE us2.id = cursor_value::uuid),
        (SELECT us2.created_at FROM user_spots us2 WHERE us2.id = cursor_value::uuid)
      ))
    )
  ORDER BY
    CASE WHEN sort_by = 'likes_count' THEN us.likes_count END DESC NULLS LAST,
    us.created_at DESC
  LIMIT result_limit;
END;
$$ LANGUAGE plpgsql STABLE;

-- 4. mixed_feed_item 複合型の spot_name を JSONB → TEXT に変更
-- 依存する関数を先にDROPする必要がある

DROP FUNCTION IF EXISTS public.generate_feed_ad_slots(integer, integer);
DROP FUNCTION IF EXISTS public.get_following_mixed_feed(uuid, integer, integer, timestamptz, timestamptz, boolean, integer, boolean);
DROP FUNCTION IF EXISTS public.get_mixed_feed(integer, integer, timestamptz, timestamptz, uuid, boolean, integer, boolean);

ALTER TYPE "public"."mixed_feed_item" ALTER ATTRIBUTE "spot_name" TYPE text;

-- 5. generate_feed_ad_slots を再作成（spot_name を NULL::TEXT に変更）
...
BEGIN
  FOR v_block IN 0..(p_num_blocks - 1) LOOP
    ...
    RETURN NEXT (
      ...
      NULL::TEXT, NULL::UUID, NULL::TEXT, NULL::INT, NULL::INT, NULL::INT,
...
-- 6. get_following_mixed_feed を再作成（spot_name を NULL::TEXT に変更）
...
-- 7. get_mixed_feed を再作成（spot_name を NULL::TEXT に変更）
