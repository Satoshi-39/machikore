-- ============================================================
-- 混合フィードのマップ・スポット取得件数を分離 + 広告スロット対応
-- ============================================================
-- 配置パターン（1ブロック = 10アイテム）:
--   map × 2 → feed_native広告 → map × 2 → spot × 4（横スワイプカルーセル、途中にcarousel_video広告）
...
-- 既存の関数を削除（戻り値の型変更のため）
DROP FUNCTION IF EXISTS public.generate_feed_ad_slots(INT, INT);
...
-- RPC関数の更新（おすすめ用）
-- ===============================
-- 配置: map×2 → feed_native → map×2 → spot×4(carousel_video含む) → ...
...
AS $$
DECLARE
  -- 1ブロック = map4 + feed_native + spot4(carousel_video含む)
...
BEGIN
  ...
  -- マップに位置番号を付与
  -- 配置: map1,map2, [feed_native], map3,map4, spot1,spot2,[carousel_video],spot3,spot4, ...
  -- 1ブロック(広告あり) = 2map + 1ad + 2map + 4spot + 1ad = 10アイテム
  -- 1ブロック(広告なし) = 4map + 4spot = 8アイテム
  -- 位置: map1=1, map2=2, ad=3, map3=4, map4=5, spot1=6, spot2=7, ad=8, spot3=9, spot4=10
  maps_with_position AS (
    SELECT
      ...
      CASE WHEN p_show_ads THEN
        -- 広告あり: map2 + ad + map2 + spot4 + ad = 10アイテム/ブロック
        p_start_position +
        ((row_num - 1) / v_maps_per_block)::INT * 10 +  -- 前のブロック分
        ((row_num - 1) % v_maps_per_block)::INT + 1 +   -- ブロック内位置(1,2,3,4)
        -- map3,map4(pos_in_block >= 2)は広告の後なので+1
        CASE WHEN ((row_num - 1) % v_maps_per_block)::INT >= 2 THEN 1 ELSE 0 END
      ELSE
        -- 広告なし: 各ブロックは8アイテム（map4 + spot4）
        p_start_position +
        ((row_num - 1) / v_maps_per_block)::INT * 8 +
  ...
  -- スポットに位置番号を付与
  -- 位置: spot1=6, spot2=7, carousel_video=8, spot3=9, spot4=10
  spots_with_position AS (
    SELECT
      ...
      CASE WHEN p_show_ads THEN
        -- 広告あり: map2 + ad + map2 = 5の後からスポット開始
        -- spot1=6, spot2=7, [carousel_video=8], spot3=9, spot4=10
        p_start_position +
        ((row_num - 1) / v_spots_per_block)::INT * 10 +  -- 前のブロック分
        5 +  -- map2 + ad + map2
        ((row_num - 1) % v_spots_per_block)::INT + 1 +
        -- carousel_videoの位置調整（spot2の後に広告が入るので、spot3以降は+1）
        CASE WHEN ((row_num - 1) % v_spots_per_block)::INT >= 2 THEN 1 ELSE 0 END
      ELSE
        p_start_position +
        ((row_num - 1) / v_spots_per_block)::INT * 8 +
        4 +  -- map4の後
        ((row_num - 1) % v_spots_per_block)::INT + 1
      END AS feed_position,
      NULL::TEXT AS ad_slot,
  ...
  -- 全て結合
  combined AS (
    SELECT item_type, item_id, created_at, feed_position, ad_slot,
           map_id, map_name, map_description, map_thumbnail_url, map_is_public, map_is_article_public,
           map_spots_count, map_likes_count, map_bookmarks_count, map_comments_count, map_category_id,
           map_language, map_user_id, map_user_username, map_user_display_name, map_user_avatar_url,
           map_tags, map_is_liked, map_is_bookmarked,
  ...
  SELECT
    item_type, item_id, created_at, feed_position, ad_slot,
    map_id, map_name, map_description, map_thumbnail_url, map_is_public, map_is_article_public,
    map_spots_count, map_likes_count, map_bookmarks_count, map_comments_count, map_category_id,
    map_language, map_user_id, map_user_username, map_user_display_name, map_user_avatar_url,
    map_tags, map_is_liked, map_is_bookmarked,
...
COMMENT ON FUNCTION public.get_mixed_feed IS '混合フィード取得 - 配置: map×2 → feed_native → map×2 → spot×4(carousel_video含む) → 繰り返し';
...
BEGIN
  ...
  -- マップに位置番号を付与
  -- 配置: map1=1, map2=2, ad=3, map3=4, map4=5, spot1=6, spot2=7, ad=8, spot3=9, spot4=10
  maps_with_position AS (
    SELECT
      item_type, item_id, created_at,
      ((row_num - 1) / v_maps_per_block)::INT AS block_num,
  ...
  spots_with_position AS (
    SELECT
      ...
      CASE WHEN p_show_ads THEN
        p_start_position +
        ((row_num - 1) / v_spots_per_block)::INT * 10 +
        5 +
        ((row_num - 1) % v_spots_per_block)::INT + 1 +
        CASE WHEN ((row_num - 1) % v_spots_per_block)::INT >= 2 THEN 1 ELSE 0 END
      ELSE
...
COMMENT ON FUNCTION public.get_following_mixed_feed IS 'フォロー中ユーザーの混合フィード取得 - 配置: map×2 → feed_native → map×2 → spot×4(carousel_video含む) → 繰り返し';
