...
function FlatMapList({ maps, currentUserId, onArticlePress }: FlatMapListProps) {
  return (
    <View>
      {maps.map((map) => (
        <MapListCard
...
export function MagazineSectionList({
  ...
  if (!hasSections) {
    ...
    return (
      ...
      >
        {headerContent}
        <FlatMapList maps={allMaps} currentUserId={currentUserId} onArticlePress={onArticlePress} />
      </ScrollView>
    );
  }
