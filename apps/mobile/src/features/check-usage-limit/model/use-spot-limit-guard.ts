 ...
 * 将来的にプレミアムアップグレード導線への差し替えもここで対応。
 */
...
export function useSpotLimitGuard() {
  ...
  // TODO: プレミアムアップグレード導線に差し替え予定
  const showSpotLimitAlert = useCallback((limit?: number) => {
    const spotLimit = limit ?? SUBSCRIPTION.FREE_SPOT_LIMIT;
    Alert.alert(
      'スポット数の上限',
