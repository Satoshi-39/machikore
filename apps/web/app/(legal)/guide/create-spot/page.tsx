      ...
      <div className="flex flex-col items-center gap-4">
        {images.map((img) => (
          <Image
            ...
            width={640}
            height={1138}
            className="rounded-xl shadow-sm max-w-[320px] w-full h-auto"
          />
        ))}
      </div>
    </div>
      ...
      <div className="flex flex-col items-center gap-4">
        {images.map((img) => (
          <Image
            ...
            width={560}
            height={994}
            className="rounded-xl shadow-sm max-w-[280px] w-full h-auto"
          />
        ))}
      </div>
    </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="bg-[var(--surface)] rounded-xl shadow-sm p-6 sm:p-8">
          ...
          <GuideStep
            step={6}
            title="スポット情報を入力する"
            description="どの方法でもスポット登録フォームに進みます。「このスポットをひとことで」（必須）と写真を入力します。"
            note="※ 検索して登録した場合はスポット名が自動入力されますが、その他の方法ではスポット名も入力が必要です。"
            images={[
              {
                src: `${IMG}/create_spot_07_A.png`,
                alt: 'スポット登録フォーム（スポット名・ひとこと・写真）',
              },
            ]}
          />
          ...
          <GuideStep
            step={9}
            title="記事編集ページを開く"
            description="「書く」を選択した場合、記事編集ページが開きます。スポット登録時に設定した写真がカバー画像として自動表示されます。また「このスポットをひとことで」で入力した内容が記事のタイトルとして表示されます（ここで編集も可能です）。"
            images={[
              {
