 ...
 * ユーザー名がメールアドレスのプレフィックスや自動生成パターンと異なれば完了
 */
function hasProfileSetupCompleted(user: User | null): boolean {
  ...
  // 自動生成パターン: user_{id} または メールプレフィックス
  const autoGeneratedPattern = `user_${user.id.slice(0, 8)}`;

  // username が自動生成されたものと異なれば設定完了とみなす
  if (username !== emailPrefix && username !== autoGeneratedPattern) {
    return true;
  }

  // display_name が username と異なれば設定完了とみなす
