 ...
 * マスタースポットのブックマークをトグル
 */
export function useToggleMasterSpotBookmark() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: ({
      ...
      masterSpotId: string;
      folderId?: string | null;
    }) => toggleMasterSpotBookmark(userId, masterSpotId, folderId),
    onMutate: async ({ userId, masterSpotId }) => {
      ...
      queryClient.setQueryData(
        ['bookmark-status', 'master-spot', userId, masterSpotId],
        !previousStatus
      );
    ...
    onError: (error, { userId, masterSpotId }, context) => {
      log.error('[Bookmark] useToggleMasterSpotBookmark Error:', error);
      Toast.show({
        type: 'error',
        text1: '保存に失敗しました',
        visibilityTime: 3000,
 ...
 * マスタースポットをブックマークに追加
 */
export function useBookmarkMasterSpot() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: ({
      ...
      masterSpotId: string;
      folderId?: string | null;
    }) => bookmarkMasterSpot(userId, masterSpotId, folderId),
    onMutate: async ({ userId, masterSpotId }) => {
      // 楽観的更新
      await queryClient.cancelQueries({
        queryKey: ['bookmark-status', 'master-spot', userId, masterSpotId],
    ...
    onError: (error, { userId, masterSpotId }) => {
      log.error('[Bookmark] useBookmarkMasterSpot Error:', error);
      Toast.show({
        type: 'error',
        text1: '保存に失敗しました',
        visibilityTime: 3000,
    ...
    onSuccess: (_, { userId, masterSpotId }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['bookmarks', userId] });
      queryClient.invalidateQueries({ queryKey: ['bookmark-status', 'master-spot', userId, masterSpotId] });
      queryClient.invalidateQueries({ queryKey: ['bookmark-info', 'master-spot', userId, masterSpotId] });
    },
  });
}
 ...
 * マスタースポットのブックマークを解除
 */
export function useUnbookmarkMasterSpot() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: ({
      ...
      userId: string;
      masterSpotId: string;
    }) => unbookmarkMasterSpot(userId, masterSpotId),
    onMutate: async ({ userId, masterSpotId }) => {
      // 楽観的更新
      await queryClient.cancelQueries({
        queryKey: ['bookmark-status', 'master-spot', userId, masterSpotId],
    ...
    onError: (error, { userId, masterSpotId }) => {
      log.error('[Bookmark] useUnbookmarkMasterSpot Error:', error);
      Toast.show({
        type: 'error',
        text1: '保存の解除に失敗しました',
        visibilityTime: 3000,
