...
- **テストのためのテストにしない** — テストインフラ自体の検証に終始しないこと
- **テスト起因のプロダクションコード修正は必ずユーザーに確認する** — テストの追加・修正によって実装コード（テストファイル以外）を変更する必要がある場合、必ず事前にユーザーの承認を得ること

### 単体テスト（Vitest）

- **対象**: 純粋関数、バリデーションロジック、データ変換・マッパー関数、ユーティリティ
- **対象外**: UIレンダリングのみのコンポーネント、薄いラッパー関数、型定義
- 外部依存はモックし、入力→出力の正しさを検証する

### 結合テスト（Vitest + MSW）

- **対象**: CRUD操作の主要パス（作成・取得・更新・削除）、hooks + API連携、楽観的更新の動作
- MSWでSupabase APIをモックし、実際のネットワークを経由せずにテストする
- エラーレスポンス時のフォールバック動作もテストする

### 課金制限テスト（Vitest）

- **対象**: フォルダ数・ブックマーク数・マップ数などプラン別の上限制限ロジック
- 境界値テスト（上限ちょうど、上限+1）と同値分割テスト（Free/Pro各プラン）を実施する
- 制限チェック関数は単体テスト、制限を含むCRUD操作は結合テストで検証する

### RLSテスト（pgTAP / Supabase Test Helpers）

- **新テーブル追加時/既存テーブル修正時は必ずRLSテストを追加/修正する**
- テーブル × ロール（anonymous / authenticated / owner / other_user）× 操作（SELECT / INSERT / UPDATE / DELETE）を網羅する
- RLSポリシー変更時は既存テストが通ることを確認する

### E2Eテスト（Maestro）

- ユーザージャーニーベースでシナリオを定義する
- シナリオ一覧は `docs/e2e-scenarios.md` で管理する
- 新機能追加時はシナリオ一覧を更新し、優先度の高いものからE2Eテストを実装する

### テスト追加の基準

- **新機能**: 単体テスト（ロジック部分）+ 結合テスト（主要パス）を追加。重要な機能はE2Eシナリオも追加
- **バグ修正**: 再発防止のリグレッションテストを追加（バグを再現するテストを先に書く）
- **DBスキーマ変更**: 該当テーブルのRLSテストを追加・更新
